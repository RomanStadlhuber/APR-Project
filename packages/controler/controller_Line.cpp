...
int main(int argc, char **argv)
{
  ...
  while (goals < 2)
  {
    ...
    vel = PID.get_linear_velocity();
    omega = PID.get_angular_velocity();

    if (PID.error(current_pos_x, current_pos_y, current_th, pose.line_pos_x[goals], pose.line_pos_y[goals], pose.line_th[goals]) < 0.08)
    {
      std::cout << "reached goal: " << goals << "(" << pose.line_pos_x[goals] << pose.line_pos_y[goals] << ")" << std::endl;
      goals++;
    }
